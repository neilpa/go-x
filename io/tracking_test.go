...
func TestTrackingReader_Seek(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			r := NewTrackingReader(reader{sr})
			if tt.skip > 0 {
				if _, err := sr.Seek(tt.skip, io.SeekCurrent); err != nil {
					t.Fatalf("strings reader: %s", err)
				}
				r.pos = tt.skip
			}
			...
			if err != nil {
				t.Errorf("read: unexpected failure: %s", err)
			}
			if string(tail) != text[off:] {
				t.Errorf("read: got %q want %q", string(tail), text[off:])
			}
		})
	}
}
